...
router.get('/search-by-name/:name', function (req, res, next) { 
    ...
    DashboardSearch.getSearchByName(search, (err, result) => { 
        if (err) {
            ...
            return res.json({ success: true, message: "Search result", project,user });
        }
    });
});
...
router.get('/dashboard-count', function (req, res, next) {
    ...
    if(req.authUser.role_id.name != 'Super-Admin'){
        organization_id = req.authUser.organization_id._id;
    }
    DashboardSearch.getDashBoardCount(organization_id, (err, result) => { 
        if (err) {
            ...
            return res.json({ success: true, message: "Dashboard count", total_users, total_projects, total_visitors });
        }
    });
});
