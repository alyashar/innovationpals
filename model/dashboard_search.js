...
module.exports.getSearchByName = function(name, callback) {
    user_query = { "name": { $regex: new RegExp(name, 'i') }, 'status': true, 'is_deleted': false, 'role_id': STUDENT_ID.toString() };
    project_query = { "project_name": { $regex: new RegExp(name, 'i') } };
    if (name == 'view-students') {
        user_query = { 'status': true, 'is_deleted': false, 'role_id': STUDENT_ID.toString() };
    } else if (name == 'view-projects') {
        project_query = {};
    }
    User.find(user_query).select('rating_id project_id name dob email organization_id project_id task_id profile_pic')
        ...
        .exec((err1, user) => {
            Project.find(project_query)
                ...
                .populate({ path: 'rating_id task_id project_category_id organization_id', select: 'average_rating actual_hours name' }).exec((err, project) => {
                    if (user && user.length) {
...
module.exports.getDashBoardCount = function(organization_id, callback) {
    ...
    User.count(user_query).exec((er, users) => {
        Project.count(project_query).exec((err, projects, ) => {
            User.count({ 'role_id': VISITOR_ID.toString(), 'status': true, 'is_deleted': false }).exec((err, visitors, ) => {
                let result = [];
                result['users'] = users;
